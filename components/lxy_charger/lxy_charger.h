...
class LXYCharger : public Component, public ble_client::BLEClientNode {
 ...
 protected:
  ...
  bool ready_() const;
  // A background status poll holds the wire, but does not block local UI edits.
  bool busy_() const { return query_pending_ || set_pending_ || deferred_request_pending_ || connection_request_id_ != 0; }
  ...
  void publish_connection_(uint32_t request_id = 0, Result result = Result::INFO);
  ...
  Result overflow_result_{Result::UNKNOWN};
  Event deferred_request_{};
