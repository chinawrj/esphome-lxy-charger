...
| `STATUS` | BLE、Web | 更新请求状态、结果与忙碌标记 |
| `RAW_STATUS` | BLE | 保留尚未解码的原始状态帧；不推断测量值 |
| `INPUT` | Button B | 仅表示输入动作，目前不触发 BLE 操作 |
...
实机验证状态：M5StickC Plus 的最小 BLE 配置和完整配置均**待验证**；ATOMS3U **尚未实机验证**。没有声明所有 16 个组合都已逐个刷机。具体构建结果应与源码提交及测试输出对应。
